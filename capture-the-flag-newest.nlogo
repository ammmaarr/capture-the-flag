...
to init-allies
  ...
  create-allies NUMBER-OF-ALLIES [
    ;set shape "person"
...
to init-enemies
  create-enemies NUMBER-OF-ENEMIES [
    ;set shape "person"
    set heading 90
    set color green
    set size 2
...
;  ask patches with [wall? = true] [
;    if any? enemy-bullets in-radius 3 or any? ally-bullets in-radius 3 [
;      set life life - 1
;      set pcolor pcolor - 1
;    ]
;  ]

;; ========== enemy movement before walls breaking implementation
;  ask enemies [
;    ifelse any? patches in-cone 10 20 with [ wall? = true ] [
;      rotate-left
;      let dleft 0
;      if any? patches in-cone 5 20 with [wall? = true] [
;        set dleft distance one-of patches in-cone 5 20 with [wall? = true]
;      ]
;      repeat 2 [ rotate-right]
;      let dright 0
;      if any? patches in-cone 5 20 with [wall? = true] [
;        set dright distance one-of patches in-cone 5 20 with [wall? = true]
;      ]
;      rotate-right
;      ifelse dleft = dright [
;       let r random 2
...
;  ]
  ...
  ask enemies [
    if any? allies in-cone 8 60 [
      enemy-shoot self
    ]
  ]
;
;  ask enemy-bullets [
;    let dying one-of allies in-radius size
;    if dying != nobody [
;      ask dying [
;        die
;      ]
