...
globals [ GREEN-SCORE BLUE-SCORE wall-life player deltax deltay bullet-speed bullet-life max-bullet-count previous went-up? went-down? goal green-flag blue-flag ]
...
to setup
  ...
  ask enemies with [strategy = "shooter" ] [
   ...
   ask allies [
    set shape "person"
    set bullet-count max-bullet-count
  ]
  ask enemies [
    set shape "person"
    set bullet-count max-bullet-count
...
to init-constants
  ...
  set bullet-life world-width / ( bullet-speed * 6 )
  set max-bullet-count 8
  set goal nobody
  set previous nobody
  set wall-life 5
end
...
to init-allies
  if player = nobody [
    create-allies 1 [
      ...
      set color black
      set size 2
      move-to one-of patches with [ not any? turtles-here and wall? = false and pxcor < max-pxcor and pxcor > (max-pxcor / 2) and pxcor < max-pxcor - 2 and pycor < max-pycor - 2]
      set player self
    ]
  ]
...
to init-enemies
  create-enemies NUMBER-OF-ENEMIES - 2 [
    ...
    set size 2
    set strategy "shooter"
    move-to one-of patches with [ not any? turtles-here and wall? = false and pxcor > min-pxcor and pxcor < (min-pxcor / 2) and pxcor < max-pxcor - 2 and pycor < max-pycor - 2 ]
  ]
...
to fwd
  if xcor <= (max-pxcor) and ycor <= ( max-pycor) and xcor >= min-pxcor and ycor >= min-pycor [
    if not [wall?] of patch-ahead 0 [
      fd 1
    ]
  ]
end
